...
(function(){
  ...
  cards.forEach(function(card){
    ...
    vid.addEventListener('loadeddata', function(){ card.classList.add('vid-ready'); });
    vid.addEventListener('error', function(){ card.classList.remove('vid-ready'); });

    function enter(){
      if(reduce) return;
      if(vid.readyState < 2) vid.load();
      var p = vid.play();
      if(p && p.catch) p.catch(function(){ /* autoplay blocked, cover stays visible */ });
    }
    function leave(){
      vid.pause();
